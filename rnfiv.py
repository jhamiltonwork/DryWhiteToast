...
def ReadWrite(elist, outfile):
    try:
        ...
        coln = ['Email']
        ecsv = pd.read_csv(elist, names=coln, encoding='utf-8')
        logging.info('Complete' + '\n')
        DisplayMessage('Complete' + '\n')
        ...
        if (ecsv.shape[1]) < 2:
            ...
            if erows > crows:
                ...
                common['Hash'] = common['Email'].encode('utf-8')
                common['Hash'] = common['Hash'].apply(lambda h: hashlib.sha256(h).hexdigest().upper().strip())
                print (common)
                #logging.info('complete' + '\n')
                #DisplayMessage('complete' + '\n')
                ...
                logging.info('Creating sha512 hash id for each email string')
                DisplayMessage('Creating sha512 hash id for each email string')
                ecsv['Hash'] = ecsv['Email'].apply(lambda h: hashlib.sha512(h).hexdigest().upper().strip())
                logging.info('complete' + '\n')
                DisplayMessage('complete' + '\n')
                logging.info('Writing hash list to csv in ' + output)
                DisplayMessage('Writing hash list to csv in ' + output)
...
if __name__ == '__main__':
    ...
    if daystoexp.days > 0:
        ...
        fpath = str(spath)
        lpath = fpath.replace('rnfiv.py', 'Toast.log')
